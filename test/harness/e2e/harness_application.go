...
// LogRootlessToRootfulTransitionDebug runs debug commands on the VM to inspect agent logs and systemd unit
// status during rootless→rootful transition. Use when the transition leaves an app in Error or the test times out.
// appNamePattern is a unit name substring or pattern (e.g. "new-rootful-app"); userSystemd is the runAs user
// for user systemd (e.g. "flightctl"), or empty to skip user unit listing.
func (h *Harness) LogRootlessToRootfulTransitionDebug(appNamePattern, userSystemd string) {
	GinkgoWriter.Printf("[rootless→rootful debug] app pattern=%q user=%s\n", appNamePattern, userSystemd)
	commands := []struct {
		label string
		args  []string
	}{
		{"journalctl -u flightctl-agent -n 100", []string{"sudo", "journalctl", "-u", "flightctl-agent", "-n", "100", "--no-pager"}},
		{"systemctl list-units (root) *" + appNamePattern + "*", []string{"sh", "-c", "sudo systemctl list-units --all --no-pager '*" + appNamePattern + "*' 2>&1"}},
		{"systemctl status (root) *" + appNamePattern + "*", []string{"sh", "-c", "sudo systemctl status '*" + appNamePattern + "*' --no-pager 2>&1"}},
	}
	for _, c := range commands {
		out, err := h.VM.RunSSH(c.args, nil)
		if err != nil {
			GinkgoWriter.Printf("[rootless→rootful debug] %s: error %v\n", c.label, err)
			continue
		}
		GinkgoWriter.Printf("[rootless→rootful debug] %s:\n%s\n", c.label, out.String())
	}
	if userSystemd != "" {
		cmd := fmt.Sprintf("sudo -u %s sh -c 'cd /tmp && XDG_RUNTIME_DIR=/run/user/$(id -u %s) systemctl --user list-units --all --no-pager 2>&1' | grep '%s' || true", userSystemd, userSystemd, appNamePattern)
		out, err := h.VM.RunSSH([]string{"sh", "-c", cmd}, nil)
		if err != nil {
			GinkgoWriter.Printf("[rootless→rootful debug] systemctl --user list-units (%s): error %v\n", userSystemd, err)
		} else {
			GinkgoWriter.Printf("[rootless→rootful debug] systemctl --user list-units (%s):\n%s\n", userSystemd, out.String())
		}
	}
}

// logRootlessDebug runs debug commands on the VM for the given user's home and writes output to GinkgoWriter.
// Use when rootless podman fails (e.g. "stat .../.config: permission denied") to inspect ownership and permissions.
func (h *Harness) logRootlessDebug(user, home string) {
	GinkgoWriter.Printf("[rootless debug] user=%s home=%s\n", user, home)
	commands := []struct {
		label string
		args  []string
	}{
		{"getent passwd " + user, []string{"sudo", "getent", "passwd", user}},
		{"ls -la " + home, []string{"sudo", "ls", "-la", home}},
		{"ls -la " + home + "/.config", []string{"sudo", "ls", "-la", home + "/.config"}},
		{"ls -la " + home + "/.config/containers 2>&1", []string{"sh", "-c", "sudo ls -la " + home + "/.config/containers 2>&1"}},
		{"id " + user, []string{"sudo", "id", user}},
		{"stat " + home + " 2>&1", []string{"sh", "-c", "sudo stat " + home + " 2>&1"}},
		{"stat " + home + "/.config 2>&1", []string{"sh", "-c", "sudo stat " + home + "/.config 2>&1"}},
	}
	for _, c := range commands {
		out, err := h.VM.RunSSH(c.args, nil)
		if err != nil {
			GinkgoWriter.Printf("[rootless debug] %s: error %v\n", c.label, err)
			continue
		}
		GinkgoWriter.Printf("[rootless debug] %s:\n%s\n", c.label, out.String())
	}
}
...
// No manual chown; the VM/agent is expected to set up the user's home with correct ownership. On failure, logs debug info.
func (h *Harness) RunPodmanPsContainerNamesAsUser(user string, allContainers bool) (string, error) {
	...
	if err != nil {
		if user != "root" {
			home, getHomeErr := h.getUserHomeOnVM(user)
			if getHomeErr == nil {
				h.logRootlessDebug(user, home)
			}
		}
		return "", err
	}
