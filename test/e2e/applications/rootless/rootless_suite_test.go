...
var _ = BeforeEach(func() {
	...
	// Ensure flightctl user has .config and .local and owns its home so rootless podman can run (avoids "stat .../.config: permission denied").
	const flightctlUser = "flightctl"
	flightctlHome, err := harness.GetUserHomeOnVM(flightctlUser)
	Expect(err).ToNot(HaveOccurred(), "getent passwd %s", flightctlUser)
	_, err = harness.VM.RunSSH([]string{"sudo", "mkdir", "-p", flightctlHome + "/.config", flightctlHome + "/.local"}, nil)
	Expect(err).ToNot(HaveOccurred(), "mkdir .config/.local for %s", flightctlUser)
	_, err = harness.VM.RunSSH([]string{"sudo", "chown", "-R", flightctlUser + ":" + flightctlUser, flightctlHome}, nil)
	Expect(err).ToNot(HaveOccurred(), "chown home .config .local for %s", flightctlUser)
