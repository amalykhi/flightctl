...
var _ = Describe("Real Hardware TPM Device Authentication", Label("hardware", "tpm", "real-tpm", "sanity", "83974"), func() {
	var (
		ctx             context.Context
		harness         *e2e.Harness
		enrollmentID    string
...
func installFlightCtlAgentFromCopr(coprRepo string) error {
	...
	if _, err := exec.LookPath("flightctl-agent"); err == nil {
		...
		GinkgoWriter.Printf("  ℹ️  FlightCtl agent already installed\n")
	} else {
